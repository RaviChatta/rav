...
class Database:
    ...
    async def add_user(self, bot, message) -> bool:
        ...
        user_data = self.new_user(user.id)
        try:
            ...
            if len(message.command) > 1 and message.command[1].startswith('refer_'):
                await self._process_referral(user.id, message.command[1][6:])
    ...
    async def create_points_link(self, admin_id: int, points: int, 
        ...
        try:
            ...
            base_url = "https://t.me/your_bot_username?start=points_"  # Change to your bot's URL
    ...
    async def _update_monthly_leaderboard(self):
        ...
        # [Implementation similar to weekly but with monthly dates]
        # ...

    async def _update_alltime_leaderboard(self):
        """Update all-time leaderboard."""
        # Points balance leaders
        points_leaders = await self.users.aggregate([
            {"$match": {"ban_status.is_banned": False}},
            {"$sort": {"points.balance": -1}},
            {"$limit": 100},
